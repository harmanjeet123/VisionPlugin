...
echo '<button class="custom-btn purevw_btn" id="select_lens" type="button">'. __('Select Lenses', 'glasses-prescription').'</button>';
...
<div class="loader"  id="loading_div"  style="display:none;">
<img src="<?php echo plugins_url('Purevision/img/eyewear.gif'); ?>">
</div>
...
<img src="<?php echo plugins_url( Plugin_folder_name().'/img/loader-horizontal.gif');?>"> 
</div> 
<div class="file__uploaded">
<button type="button" id="removeImage1" value="x" class="btn-rmv1"><img width="25" src="<?php echo plugins_url( Plugin_folder_name().'/img/icons8-remove-48.png');?>"> </button>
...
<fieldset>
   <div class="choose_tint_lens_div_box step" id="lens_finish" style="display: none;">
      <div id="rx-prescription-container" class="rx-prescription-container none" style="display: block;">                      
         <div role="right-wrap">
            <div role="lens-option-type" class="tint_color">
               <h4>Choose Tint Color Options</h4>
               <div role="tint-color">
                  <h6>Tint color:</h6>
                  <?php
                  global $wpdb;
                  ...
                  $lens_tint_tbl = $wpdb->prefix . 'purevision_lens_tints';
                  $lens_tint_results = $wpdb->get_results("SELECT * FROM $lens_tint_tbl");?>
                  <ul>
                 <?php foreach($lens_tint_results as $lens_finish => $value)
                  {
                     ?>
                
                     <li>
                     <div class="tint-color-section" style="background-color:<?php echo $value->name;?>;">
                     <span class="blue-round checked-color" role="color-pick"><?php echo $value->name;?></span>
                     <input type="hidden"  id="<?php echo $value->id; ?>" class="gradientcolor" data-name="<?php echo $value->name; ?>" name="gradientcolorvalue" value = "<?php echo $value->name; ?>"> 
                     </div></li>
                  
                  <?php } 
                  ?>
                  </ul>
                  <input type="hidden"  id="gradientcolorvalue" class="gradientcolorvalue"  name="gradientcolor" value = ""> 
                     
               </div>
            </div>
           <div role="lens-option-type">
               <div role="tint-color">
                  <h6>Tint Density:</h6>
                  <div role="tint-density">
                     <?php
                     $lens_tint_density = $wpdb->prefix . 'purevision_tint_density';
                     $lens_tint_results = $wpdb->get_results("SELECT * FROM $lens_tint_density");
                        foreach($lens_tint_results as $lens_finish => $value)
                        { 
                           if(!empty($value->id)){
                     ?>

                     <div role="density-col" class=" tintoption">
                        
                        <input type="hidden" name="tint_density" value="<?php echo $value->percentage ?>" class="tint_density" data-name="<?php echo $value->percentage ?>">
                        <span class="density-val"><?php echo $value->percentage ?>%</span><span>
                           <p>
                           <?php echo $value->text ;?>
                     ...
                     ?>
                      <input type="hidden" name="tint_density_value"  class="tint_density_value" value="">
                        
                     <div role="density-col" class="tintextraoption">
                        <span role="other-price">Others Percentage</span>
                        <select id ="tintOtherOption" onchange = "myFunction()">
                        <option value = "">Select Percentage</option>
                       <?php foreach($lens_tint_results as $lens_finish => $value)
                        {?>
                           <option value = "<?php echo $value->others ;?>"><?php echo $value->others ;?>%</option>
                           <?php } ?>
                        </select> 
                        
                        <input type="hidden" name="options_density"  class="options_density" value="">
                     </div>
                  </div>
               </div>
            </div>
            <div role="lens-option-type">
               <div role="tint-color" class="tint-colr">
               <input type = "checkbox" name = "gradient" class = "gradient" value = "gradient tint" >
                  <h6>Add Gradient to the Tint</h6>
                  <div role="gradient-tint">
                     <p>
                        Full tint at the top, fading <br>
                        gradually to no tint at the bottom.
                     </p>
                  </div>
               </div>
            </div>
            <?php
               ...
               ?>
            <a href="#" class="custom-btn purevw_btn mt-3 nexttab" >Proceed to next step</a>
...
?>
<li role="box" >
<input type="checkbox"  id="<?php echo $value->lens_finish_id; ?>" class="purevw_lens_finish_box" data-name="<?php echo ucfirst($value->lens_finish_name); ?>" name="lens_finish_pv" value = "<?php echo ucfirst($value->lens_finish_name); ?>">
<?php
   ...
   <div role="stepper-step">
                  <div role="upgardes">
                     ...
                     <div role="tint-color">
                        <h6>Tint Density:</h6>
                        <div role="tint-density">
                           <div role="density-col" class="  add-on-select">
                              <h4>UV Protection</h4>
                           ...
                           <div role="density-col" class="add-on-coatings">
                              ...
                              <div role="additional-text">
                                 ...
                                 <button class="not_included add_oncaoting_price" data-price="5" >Add <?php echo get_woocommerce_currency_symbol(); ?>5</button>
...
function add_cart_item_data( $cart_item_data, $product_id, $variation_id ) {
   ...
   $tint_density        = $_POST['tint_density_value'];
   $extra_price         = $_POST['options_density'];
   ...
   if($lens_Gradient == ''){}else{$cart_item_data['gradient']       = $lens_Gradient;}
   if($tint_density == ''){}else{$cart_item_data['tint_density']   = $tint_density;}
   if(empty($extra_price)){} else{$cart_item_data['extra_price']   = $extra_price;}
   if ($options_addons == 0){}	
   if ($options_addons > 0){
...
add_filter( 'woocommerce_get_item_data', 'display_cart_item_custom_meta_data', 10, 2 );
      function display_cart_item_custom_meta_data( $item_data, $cart_item ) {
         ...
         $Lens_gradientcolor = 'Lens gradient color';
         $Lens_gradient      = 'Lens gradient';
         $tint_density      = 'Tint Density';
         $options_density    = 'Tint Other Percentage'; 
        if ( isset($cart_item['lens_type'])  ) {
         ...
         if ( isset($cart_item['gradientcolor'])  ) {
            $item_data[] = array(
               'key'       => $Lens_gradientcolor,
               'value'      => $cart_item['gradientcolor'],
            );
         }
         if ( isset($cart_item['gradient'])  ) {
            $item_data[] = array(
               'key'       => $Lens_gradient,
               'value'      => $cart_item['gradient'],
            );
         }
         if ( isset($cart_item['tint_density'])  ) {
            $item_data[] = array(
               'key'       => $tint_density,
               'value'      => $cart_item['tint_density'].'%',
            );
         }
         if ( isset($cart_item['extra_price'])  ) {
            $item_data[] = array(
               'key'       => $options_density,
               'value'      => $cart_item['extra_price'].'%',
            );
         }
...
function bbloomer_quantity_based_pricing( $cart ) {
 foreach ( $cart->get_cart() as $cart_item_key => $cart_item ) {
     ...
     if(!empty($cart_item['tint_density'])){
      $tint = $cart_item['tint_density'];
     }
     else{
      $tint = 0;
     }
     if(!empty($cart_item['extra_price'])){
      ...
      if (!empty($cart_item['lens_choose']) ) {
         $price = round( (int)$cart_item['data']->get_price() + (int)$cart_item['lens_choose'] +$option + $tint + $extra_price);
...
function lens_order_meta_handler( $item_id, $values, $cart_item_key ) {
   ...
   if( isset( $values['gradientcolor'] ) ) {
      wc_add_order_item_meta( $item_id, "lens Gradient color", $values['gradientcolor'] );
   }
   if( isset( $values['gradient'] ) ) {
         wc_add_order_item_meta( $item_id, "lens gradient", $values['gradient'] );
     }
     if( isset( $values['tint_density'] ) ) {
      wc_add_order_item_meta( $item_id, "lens tint density", $values['tint_density'] );
  }
  if( isset( $values['extra_price'] ) ) {
   wc_add_order_item_meta( $item_id, "lens extra price", $values['extra_price'] );
   }
}
