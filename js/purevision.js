...
function myFunction() {
  window.myvar = jQuery("#tintOtherOption").val();
  jQuery('.options_density').val(window.myvar);
}
...
jQuery(document).ready(function () {
  ...
  var lens_tint = jQuery(".choose_tint_lens_div_box");
  ...
  jQuery(document).on("click", ".purevw_tint_lens_box", function () {
    lens_tint.hide();
    choose_lens.show(); 
    currentScreen = lens_tint;
  });
  ...
  jQuery(document).on("click", "#prev", function () {
    if (currentScreen == add_priscription) {
      ...
      lens_material.show(200);
      currentScreen = lens_material;
    } else if (currentScreen == lens_tint) {
      lens_tint.hide(200);
  ...
  jQuery(document).on("click", ".purevw_lens_finish_box", function () {
    lensTint = jQuery(this).val();
    console.log(lensTint);
    if(lensTint == "Tint"){
      lens_finish.hide();
      choose_lens.hide();
      step_to_checkout.hide();
      lens_tint.show();
      currentScreen = lens_tint;
    }
    else{
  ...
  jQuery('.tintoption').click(function(){
    jQuery('.tintoption.density-active').removeClass("density-active");
    jQuery(this).addClass("density-active");
    
    density = jQuery(this).find('.tint_density').data("name");
    jQuery('.tint_density_value').val(density);
    total_price = + parseFloat(salePrice).toFixed(2) + +parseFloat(density).toFixed(2);
    jQuery(".PV_finalPrice").html(parseFloat(total_price + density).toFixed(2));
    ...
    jQuery('.tintextraoption').click(function(){
      ...
      density = jQuery('.tintoption.density-active').find('.tint_density').data("name");
      jQuery('.tint_density_value').val(density);
      console.log(density);
      total_price = + parseFloat(salePrice).toFixed(2) + +parseFloat(density).toFixed(2) + +parseFloat(costTint).toFixed(2);
  ...
  jQuery('.choose_purevision').click(function () {
    ...
    if(density > 0){
    total_price = + parseFloat(salePrice).toFixed(2) + +parseFloat(price).toFixed(2) + +parseFloat(density).toFixed(2)+ +parseFloat(costTint).toFixed(2);
    console.log(parseFloat(total_price).toFixed(2));
    }
    else{
    total_price = + parseFloat(salePrice).toFixed(2) + +parseFloat(price).toFixed(2) + +parseFloat(costTint).toFixed(2);
    console.log(parseFloat(total_price).toFixed(2));
    }
    // jQuery('.').html();
    if (jQuery("span").hasClass('PV_finalPrice')) {
 ...
 jQuery('.tint-color-section').click(function () {

  jQuery('.tint-color-section.density-active').removeClass("density-active");
  jQuery(this).addClass("density-active");
  gredient = jQuery(this).find('.gradientcolor').data("name");
  jQuery('.gradientcolorvalue').val(gredient);
  jQuery('.tint-color-section.density-active').css('border','3px solid ' + gredient); 
});
jQuery('.add_oncaoting_price').click(function(e){
  e.preventDefault();
  if (jQuery('.add-on-coatings').hasClass("density-active")) {
    jQuery('.add-on-coatings').removeClass("density-active");
   } else {
    jQuery('.add-on-coatings').addClass("density-active");
  }
